...
func main() {
	...
	bridge := exec.Command("go", "run", "./cmd/foxstream-bridge")
	...
	go func() {
		defer close(done)
		for {
			msg, err := protocol.ReadMessage(stdout)
			if err != nil {
				if err != io.EOF {
			...
			if msgType == "complete" || msgType == "error" || msgType == "pong" {
				return
			}
		}
	}()
	...
	switch os.Args[1] {
	case "ping":
		sendMessage(stdin, map[string]any{"action": "ping"})
	...
	case "direct":
		...
		sendMessage(stdin, map[string]any{
			"action": "download", "id": downloadID,
			"url": os.Args[2], "streamType": "mp4", "title": "Test Direct Download",
		})
	...
	case "hls":
		...
		sendMessage(stdin, map[string]any{
			"action": "download", "id": downloadID,
			"url": os.Args[2], "streamType": "hls", "streamStructure": "muxed", "title": "Test HLS Download",
		})
	...
	case "demuxed":
		...
		sendMessage(stdin, map[string]any{
			"action": "download", "id": downloadID,
			"url": os.Args[2], "streamType": "hls", "streamStructure": "demuxed", "title": "Test Demuxed Download",
		})
	...
	if downloadID != "" {
		...
		go func() {
			...
			sendMessage(stdin, map[string]any{"action": "cancel", "id": downloadID})
		}()
	}
...
func sendMessage(w io.Writer, msg map[string]any) {
	...
	fatal(err, "marshal")
	fmt.Printf("[SEND] %s\n", string(data))
	fatal(protocol.WriteMessage(w, data), "write message")
}
